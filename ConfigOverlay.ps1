...
function Get-ConfigOverlayReadmeText {
    ...
    $lines = @(
        ...
        '  false to write A..H data only. The columns are I 処理時間(検算) /',
        '  J チェック / K 件数(参照) / L 件数チェック. CountReference.* configures',
        '  K: Enabled pulls each job''s EXPECTED record count out of an external',
        '  workbook (Directory/FileName/SheetName, {Tag} and {Month} tokens;',
        '  KeyColumn matched against the first KeyLength chars of the correl id,',
        '  ValueColumn returned; FirstRow/LastRow bound the ranges). Left disabled,',
        '  K holds an inert TEXT placeholder with <DIR>/<BOOK>/<SHEET> tokens to',
        '  search-and-replace later (PlaceholderWhenUnset false empties it). L',
        '  compares the OCR-read count against K when K has a value, else against',
        '  the same correl''s OTHER SIDE (GIFT vs GFIX): equal reads OK, including',
        '  0 vs 0; blank when there is nothing to compare against.',
        '  Stage (Ocr/Write/Both, default',
        ...
        '  SnapDirPattern ({0}=GIFT/GFIX), PromoteIdentifiedSnap (default true --',
        '  once OCR identifies which exported evidence picture belongs to a',
        '  correl, save it under the canonical snap name so the hyperlink, the',
        '  verify column and later runs all find it; never overwrites a real',
        '  capture and never writes a .txt), FallbackImage (default true -- the',
        '  older ranked guess among exported pictures, still used for rows whose',
        '  cached OCR result predates PromoteIdentifiedSnap),',
        '  RenderFont, and PixelDiff.Enabled --',
