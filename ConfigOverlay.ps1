...
function Get-ConfigOverlayReadmeText {
    ...
    $lines = @(
        ...
        '  separate JDL/JRV workbooks (classified per correl by whether its mapping',
        '  Excel_NAME contains "JDL" or "JRV"). AnchorCol matches Replace.ColAnchor;',
        ...
        '  [char] label. OcrLanguage is the secondary Windows OCR language pooled',
        '  alongside en-US when no archived snap\GIFT_HM|GFIX_HM\<correl>.txt is',
        '  available. Stage (Ocr/Write/Both, default Both) picks which of the extract-',
        '  and-cache / write-the-output-workbooks stages -Phase ProcessTime runs by',
        '  default; CLI -Stage / the ''stage'' menu option override it per run.',
        '',
