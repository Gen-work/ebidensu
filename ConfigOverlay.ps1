...
function Get-ConfigOverlayReadmeText {
    ...
    $lines = @(
        ...
        '  false to write A..H data only. CountReference.* (v2.18.0) turns that K',
        '  check into the real one: Enabled pulls each job''s EXPECTED record count',
        '  out of an external workbook (Directory/FileName/SheetName, {Tag} and',
        '  {Month} tokens; KeyColumn matched against the first KeyLength chars of',
        '  the correl id, ValueColumn returned; FirstRow/LastRow bound the ranges)',
        '  into a new K 件数(参照) column and compares it in L 件数チェック.',
        ...
        '  SnapDirPattern ({0}=GIFT/GFIX), FallbackImage (default true -- a row',
        '  with no standalone snap PNG links the picture exported out of the',
        '  evidence workbook instead of getting no hyperlink at all),',
