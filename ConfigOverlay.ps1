...
function Get-ConfigOverlayReadmeText {
    ...
    $lines = @(
        ...
        '- ProcessTime.*: HM processing start/end/duration extraction (GIFT+GFIX) into a',
        '  standalone evidence workbook. AnchorCol matches Replace.ColAnchor; OutputPath/',
        '  OutputSheetName default to <WorkDir>\ProcessTime_<Owner>.xlsx / a [char] label;',
        '  OcrLanguage is the secondary Windows OCR language pooled alongside en-US when',
        '  no archived snap\GIFT_HM|GFIX_HM\<correl>.txt is available. Stage (Ocr/Write/',
        '  Both, default Both) picks which of the extract-and-cache / write-the-output-',
        '  workbook stages -Phase ProcessTime runs by default; CLI -Stage / the ''stage''',
        '  menu option override it per run.',
        '',
