...
$cfg = switch ($Mode) {
    'GiftRecv' { @{ Field='GIFT_Jenkins_snap';    Folder='GIFT_Jenkins';    UrlKind='GIFT'; Download=$true;  DataDir='DATA\GIFT' } }
    'GfixRecv' { @{ Field='GFIX_Jenkins_snap';    Folder='GFIX_Jenkins';    UrlKind='GFIX'; Download=$true;  DataDir='DATA\GFIX' } }
    'NoGfix'   { @{ Field='GIFT_noGfixfile_snap'; Folder='GIFT_noGfixfile'; UrlKind='GFIX'; Download=$false; DataDir=$null      } }
}
...
Write-Host ("  Download    : {0}{1}" -f $cfg.Download, $(if ($cfg.Download) { " -> $($cfg.DataDir)" } else { "" }))
...
$dataDir = $null
if ($cfg.Download) {
    $dataDir = Join-Path $WorkDir $cfg.DataDir
