...
改完文档跑一下 `powershell -File Tests\Run-Tests.ps1` —— `Test-Docs.ps1` 会检查
已废弃的旧名字、四处卡数是否一致、跨文件的章节引用是否存在、工作流示例里
引用的 step 是否定义过。这四类问题在评审里反复出现过,现在由机器兜底。

**当前共 115 张卡**(到 P3 可接新工作的最小集是 79 张)。开工前有四轮
审查追加的 **`P0-R1`…`P0-R17` 修订卡**(在 BACKLOG.md 最前面,`P0-R` 前缀,
...
机制)。P0 的写码卡也基本落地:P0-07 runner spike `[x]`,P0-08 的三个 step +
`workflows/spike.capture.json` 已提交、只差在**办公 PC** 上真跑一次存下 PNG,
P0-01 的标签只差在有推送权限的机器上推一下(命令在卡里)。**这两件做完就
可以开 P1**(从 P1-01 / P1-02 / P1-35 起)。
