...
foreach ($g in $groups) {
    ...
    # Try in order: <SourceDir>\<bizcode>\<name>.xlsx (exact)  ->
    #               <SourceDir>\<bizcode>\*<name>.xlsx (suffix) ->
    #               <SourceDir>\<name>.xlsx (exact, flat) ->
    #               <SourceDir>\*<name>.xlsx (suffix, flat).
    # First match wins. If a directory has multiple suffix matches,
    # pick the most-recently-modified and log the ambiguity.
    $foundPath = $null
    $foundFrom = ''
    $ambigCount = 0
