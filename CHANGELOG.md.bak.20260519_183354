...
- Anchor-row math relies on Excel COM returning accurate `pic.Top` and `pic.Height` immediately after `AddPicture`. If a picture comes back with `Height = 0` (rare, but happens if image file is corrupted) the next anchor row will collapse onto the same row.
- `Reset-SheetBelowRow` deletes shapes by `top >= rowTop - 1.0`. If the user manually placed a shape that vertically straddles the boundary, it will be deleted. Acceptable for now since the row-3-down area is supposed to be tool-managed.
- `Ensure-Column` only mutates `$allRows` in memory; the column is persisted on the next `Export-Csv`. If a Replace run finds zero matches and writes nothing, the column does not get added that run.
- `ReplaceGfix` log section currently writes a single placeholder line per correl. Real implementation will produce multi-line output; row count after that section will grow.
